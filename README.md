...
All writes (add / edit / archive / +1 / −1 / photo upload) go through server API routes.
The database is never touched directly from the browser — the server uses the Supabase
service-role key, and the tables have RLS enabled with no public policies.
...
Open <http://localhost:3000/admin/items>.

## 3. Deploy to Vercel

1. Push this repo to GitHub.
2. In [Vercel](https://vercel.com), **Add New → Project** and import the repo.
3. Under **Environment Variables**, add the same keys:
...
- Keep `SUPABASE_SERVICE_ROLE_KEY` server-side only (it is never imported into a client
  component here).
- There is no login or PIN — anyone with the URL can adjust stock. That's the intended
  simplicity for a small trusted team.
